...

{
    ...
    //    BOOL _isMoving;
    //    NSTimer *_timer;
    LZHomeGridViewListItemView *_currentPressedView;
...
- (void)setGridModelsArray:(NSArray *)gridModelsArray
{
    ...
    [gridModelsArray enumerateObjectsUsingBlock:^(LZHomeGridItemModel *model, NSUInteger idx, BOOL *stop) {
        LZHomeGridViewListItemView *item = [[LZHomeGridViewListItemView alloc] init];
        ...
        item.iconViewClickedOperationBlock = ^(LZHomeGridViewListItemView *view){
            [weakSelf deleteView:view];
        };
        item.buttonClickedOperationBlock = ^(LZHomeGridViewListItemView *view){
...
{
    LZHomeGridViewListItemView *pressedView = (LZHomeGridViewListItemView *)longPressed.view;
    CGPoint point = [longPressed locationInView:self];
    //    CGPoint superViewPoint = [longPressed locationInView:self.superview];
    //    if (point.y >= self.LZ_height - 20) {
    //        [self setContentOffset:CGPointMake(0, self.contentSize.height - self.LZ_height) animated:YES];
    //    } else if (self.contentOffset.y == self.contentSize.height - self.LZ_height) {
    //        [self scrollsToTop];
    //    }
    ...
    /*
     ...
     if ((point.y - _lastPoint.y) > 0 && point.y >= self.LZ_height - 20 && self.contentOffset.y != self.contentSize.height - self.LZ_height) {
     _isMoving = YES;
     NSLog(@">>>>>>>>>>> %@", NSStringFromCGPoint(point));
     //        [UIView animateWithDuration:.4 animations:^{
     //            self.contentOffset = CGPointMake(0, self.contentSize.height - self.LZ_height);
     //        }];
     value = self.contentSize.height - self.LZ_height;
     ...
     _timer = timer;
     } else if ((point.y - _lastPoint.y) < 0 && point.y <= self.contentSize.height - self.LZ_height + 20) {
     _isMoving = YES;
     NSLog(@">>>>>>>>>>>****** %@", NSStringFromCGPoint(point));
     [UIView animateWithDuration:.4 animations:^{
...
- (void)deleteView:(LZHomeGridViewListItemView *)view
{
...
- (void)saveItemsSettingCache
{
    ...
    [_itemsArray enumerateObjectsUsingBlock:^(LZHomeGridViewListItemView *button, NSUInteger idx, BOOL *stop) {
        if ([button isKindOfClass:[LZHomeGridViewListItemView class]]) {
            [tempItemsContainer addObject:@{button.itemModel.title : button.itemModel.imageResString}];
        }
    }];
    [LZGridItemCacheTool saveItemsArray:[tempItemsContainer copy]];
...
{
    CGFloat itemW = self.width / kHomeGridViewPerRowItemCount;
...
- (void)layoutSubviews
{
    ...
    _cycleScrollADViewBackgroundView.frame = CGRectMake(0, itemH * kHomeGridViewTopSectionRowCount, self.width, itemH);
    _cycleScrollADView.frame = CGRectMake(0, _cycleScrollADViewBackgroundView.y + 10, self.width, itemH - 10 * 2);
}
